/* first-fit allocator, in the shape of 9front's xalloc.c: one hole
 * list, seeded by pmm_add with the linear memory mem.c grew. Address
 * ordered, so a free merges with its neighbours -- without that the
 * list grows without bound and first-fit walks all of it.
 *
 * Sizes stay multiples of ALIGN, which is sizeof(struct pmm_block), so
 * a block that cannot be split is exactly the size asked for.
 */
