/* the slice of the EFI Boot Services / System Table that kernel.c is
 * written against: one periodic timer event and ConIn's
 * ReadKeyStroke/WaitForKey. There is no firmware here, so this defines
 * exactly the members kernel.c dereferences, named the same, and
 * clock.c backs them with the host's monotonic clock and its wait.
 */
#ifndef WASM_EFI_H
#define WASM_EFI_H
