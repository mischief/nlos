...
/* what a machine keeps in hand before it is called short: a
 * sixteenth. The figure is headroom rather than a threshold anything
 * measures, and what it has to exceed is one lap's allocation by
 * whatever is still running -- below that a proc dies for memory the
 * next release would have found.
 */
